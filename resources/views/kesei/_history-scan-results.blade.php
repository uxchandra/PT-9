    ...
    <table class="min-w-full text-xs whitespace-nowrap border-separate border-spacing-0">
        <thead>
            <tr class="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                ...
                <th class="sticky top-0 z-10 bg-gray-50 px-4 py-3 border-b-2 border-l border-gray-300">{{ __('Qty KBN') }}</th>
        ...
        <tbody>
            @forelse ($scans as $scan)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 border-b border-l border-gray-300 text-gray-600">{{ $qtyKbn[$scan->part_no] ?? '-' }}</td>
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-6 py-8 text-center text-gray-400">{{ __('Belum ada data scan.') }}</td>
                </tr>
            @endforelse
        </tbody>
    </table>

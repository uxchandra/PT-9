    ...
    <div class="space-y-3">
        @foreach ($locations as $slug => $name)
            <a href="{{ route('scanner.location', $slug) }}"
               class="tap flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm active:shadow-none">
                ...
                <span class="min-w-0 flex-1 truncate text-base font-extrabold uppercase tracking-wide text-slate-900">{{ $name }}</span>
                <svg class="ml-auto h-5 w-5 shrink-0 text-slate-300" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">

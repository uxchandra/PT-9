<x-app-layout>
    <x-slot name="header">
        {{ __('Edit Assignment Mesin') }} — {{ $pattern->patternBoard->name }}
    </x-slot>
        ...
        <div class="max-w-lg bg-white border border-gray-100 shadow-sm rounded-2xl p-6">
            <form method="POST" action="{{ route('patterns.update', $pattern) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <select id="machine_id" name="machine_id" required
                        ...
                        @foreach ($machines as $machine)
                            <option value="{{ $machine->id }}" @selected(old('machine_id', $pattern->machine_id) == $machine->id)>{{ $machine->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="proses" name="proses" type="number" min="1" class="block w-full mt-1" :value="old('proses', $pattern->proses)" required />
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('pattern-boards.index', ['board' => $pattern->pattern_board_id]) }}" class="text-sm text-gray-600 hover:text-gray-900">{{ __('Batal') }}</a>
                </div>
            </form>
        </div>
    </div>

<x-app-layout>
    ...
    <div class="p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        @can('view andon')
            @if ($andonPreviewBoard)
                <a href="{{ route('andon.show', $andonPreviewBoard) }}" target="_blank"
                   class="group block bg-white border border-gray-100 shadow-sm rounded-2xl overflow-hidden hover:shadow-md hover:border-brand-300 transition-all duration-200">
                    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                        <div>
                            ...
                            <p class="mt-0.5 text-sm text-gray-500">{{ __('Live preview status produksi :board — klik untuk buka penuh', ['board' => $andonPreviewBoard->name]) }}</p>
                        </div>
                    ...
                    <div class="relative bg-slate-100 overflow-hidden" style="height: 320px;">
                        <iframe src="{{ route('andon.show', $andonPreviewBoard) }}"
